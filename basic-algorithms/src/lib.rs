...
mod basic {
    ...
    fn linear_search<T: Eq>(arr: &[T], cmp_elem: T) -> usize {
    ...
    fn binary_search<T: Ord>(arr: &[T], cmp_elem: T, idx_0: usize) -> usize {
        ...
        if arr.len() > 1 {
            ...
            loop {
                ...
                if left_test && right_test {
                    // Apply recursively to partitions
    ...
    #[test]
    fn test_merge_sort() {
        // Input with even number of element
        let mut arr = ARRAY_I32_UNSORTED.clone();
        merge_sort(&mut arr);
        assert_eq!(arr, ARRAY_I32_SORTED);
    }
    ...
    #[test]
    fn test_merge_sort2() {
        // Input with odd number of element
        let mut arr = ARRAY_I32_UNSORTED2.clone();
        merge_sort(&mut arr);
        assert_eq!(arr, ARRAY_I32_SORTED2);
    }
