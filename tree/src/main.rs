...
fn walk(dir: &Path,            // Path to be used as the root
	...
	for (idx, entry) in entries.iter().enumerate() {
	    ...
	    if entries.len() - idx == 1 { // Second-last entry
		if path.is_dir() || (path.is_file() && ! directories_only) {
		    println!("{}{}{}", &filler, SYMB_TFT, &path.file_name().unwrap().to_str().unwrap());
		}
		if path.is_dir() {
		    filler_new.push_str(SYMB_FFF);
		    walk(&path, max_depth, depth+1, &filler_new, all, directories_only)?;
		}
	    } else { // All entries except the second-last
		if path.is_dir() || (path.is_file() && ! directories_only) {
		    println!("{}{}{}", &filler, SYMB_TTT, &path.file_name().unwrap().to_str().unwrap());
		}
		if path.is_dir() {
...
fn main() -> Result<()> {
    ...
    walk(&opt.dir, opt.max_depth, 0, "", opt.all, opt.directories_only)
}
